...
import {
  useGetAuthorDataOfDataset,
  useGetNormalizedData,
} from "webServices/queries";
import Tabs from "ui-components/Web/Tabs";

export default function DataFromDataset({ _id, datasetType }) {
  const { authorData, loading: authorDataLoading } =
    useGetAuthorDataOfDataset(_id);
  const { normalizedData, loading } = useGetNormalizedData(_id, datasetType);

  if (loading || authorDataLoading) {
    return (
      <div>
        <Circular />
      </div>
