...
async function process(data, search = "", nav = () => {}) {
  ...
  if (DataVerifier.isValidArray(data)) {
    data.forEach((operon) => {
      ...
      const genes = DataVerifier.isValidNumber(operon.statistics?.genes)
        ? operon.statistics?.genes
        : 0;
      const promoters = DataVerifier.isValidNumber(operon.statistics?.promoters)
        ? operon.statistics?.promoters
        : 0;
      const transcriptionFactors = DataVerifier.isValidNumber(
        operon.statistics?.transcriptionUnits,
      )
        ? operon.statistics?.transcriptionUnits
        : 0;
      table.data.push({
        ...
        name: (
          <p
            value={operonName}
            dangerouslySetInnerHTML={{ __html: operonName }}
          />
        ),
        ...
        promoters: (
          <p
            value={promoters}
            dangerouslySetInnerHTML={{ __html: promoters }}
          />
        ),
        TUs: (
          <p
            value={transcriptionFactors}
            dangerouslySetInnerHTML={{ __html: transcriptionFactors }}
          />
        ),
        _properties: {
          onClick: () => {
...
export default function Home({ query }) {
  ...
  const { loading: objectListLoading, error } = useGetObjectList({
    datamartType: "operon",
    onCompleted: (data) => {
      process(data.getObjectList, state.search, nav)
        .then((gene) => {
          dispatch({
            type: DISPATCH.SET_GENE_LIST,
            geneList: gene.list,
