...
const views = {
  list: {
    label: "List",
    tooltip: "view regulons in list",
    icon: <TableRowsIcon />,
  },
  table: {
    label: "Table",
    tooltip: "view regulons in table",
    icon: <TableChartIcon />,
  }
}

export default function RegulonViews({ loading = false, dispatch, state }) {
        ...
        <div>
          {loading ? (<>
            ...
            <Typography variant="relevant" component="div">
              {state.resultsSearch === null ? state.regulonList.length : state.resultsSearch.list.length}{" operons"}
            </Typography>
          )}
        </div>
        <div>
      ...
      {loading ? (<LinearProgress />) : (
        <div id='filterTableContainer' style={{ height: "100%" }} >
          {state.viewType === VIEW_TYPE.LIST && (
            <List
              data={state.resultsSearch === null ? state.regulonList : state.resultsSearch.list}
          ...
          {state.viewType === VIEW_TYPE.TABLE && (
            <FilterTable
            selection='row'
              data={state.resultsSearch === null ? state.regulonTable.data : state.resultsSearch.table.data}
              columns={state.resultsSearch === null ? state.regulonTable.columns : state.resultsSearch.table.columns}
              idContainer="filterTableContainer"
              pagination />
          )}
        </div>
